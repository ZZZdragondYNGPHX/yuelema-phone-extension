...
const requiredFiles = [
    ...
    'src/mvu/json-pointer.js', 'src/mvu/relationship-narrative.js', 'src/mvu/nsfw-consent.js', 'src/mvu/body-relationship-candidate.js', 'src/mvu/controlled-patch.js', 'src/mvu/adapter.js', 'src/mvu/readiness.js', 'src/mvu/test/nsfw-consent.test.mjs', 'src/mvu/test/body-relationship-candidate.test.mjs', 'src/mvu/test/mvu-adapter.test.mjs', 'src/mvu/test/readiness.test.mjs',
...
if (manifest.version !== '1.0.17') fail('manifest.version 必须与扩展版本 1.0.17 统一');
...
const productionSource = sourceText.filter((_text, index) => !/(?:^|\/)test(?:\/|$)|\.test\.mjs$/u.test(sourceRelativeFiles[index])).join('\n');
...
if (/\bstat_data\s*[.=]/.test(productionSource)) fail('禁止直接写 stat_data');
else console.log('✓ 禁止直接写 stat_data');
...
if (!appShellCore.includes("const UI_VERSION = '1.0.17'")) fail('关于软件 UI_VERSION 必须与扩展版本 1.0.17 统一（必须位于壳层 app-shell.js）');
...
const groupForumStore = await readFile(resolve(root, 'src/groups/group-forum-store.js'), 'utf8');
const localConversationSummaryService = await readFile(resolve(root, 'src/groups/local-conversation-summary-service.js'), 'utf8');
for (const [label, source] of [['群聊服务', groupChatService], ['论坛服务', forumService], ['本地对话总结服务', localConversationSummaryService]]) {
    for (const [pattern, message] of [
        [/\bfetch\s*\(/, '不得直接 fetch'],
...
if (!groupChatService.includes('generateGroupChatReply') || !groupChatService.includes('generateGroupChatUpdate') || !forumService.includes('generateForumPostDraft') || !forumService.includes('generateForumHomeRefresh') || !forumService.includes('generateForumExistingPostsUpdate') || !forumService.includes('generateForumPostConversationUpdate') || !groupLlmSafety.includes('buildPublicGroupLlmContext') || !groupLlmSafety.includes('isSafeGroupLlmOutput')) fail('缺少群聊/论坛更新服务或公开 LLM 安全投影');
if (!groupForumStore.includes('createGroupForumStore') || !groupForumStore.includes('GROUP_FORUM_STORAGE_KEY') || !groupForumStore.includes('FORUM_CHANNELS') || !groupForumStore.includes('isKnownForumChannelTopic') || !groupForumStore.includes('appendGroupModelUpdate') || !groupForumStore.includes('clearGroupHistory') || !groupForumStore.includes('exitGroup') || !groupForumStore.includes('setGroupBinding') || !groupForumStore.includes('setForumAuto') || !groupForumStore.includes('replaceForumPosts') || !groupForumStore.includes('updateExistingForumPosts') || !groupForumStore.includes('saveConversationSummary') || !groupForumStore.includes('NON_ADULT_PROFILE')) fail('缺少浏览器本地群组/论坛存储、频道或成年人校验边界');
if (!localConversationSummaryService.includes('generateLocalConversationSummary') || !localConversationSummaryService.includes("resolveFunction('chat_summary'")) fail('缺少群组/帖子本地总结的 chat_summary 绑定');
if (!index.includes('createGroupForumStore') || !appShell.includes('bindForumPullToRefresh') || !appShell.includes('forumWheelPullState') || !appShell.includes('forumInteractionAbortController') || !appShell.includes('forumPostsForActiveChannel') || !appShell.includes("'data-forum-channel'") || !appShell.includes("'wheel'") || !appShell.includes('forumIsAtTop') || !appShell.includes('forumIsAtBottom') || !appShell.includes('openGroupSettingsDialog') || !appShell.includes('setGroupBinding') || !appShell.includes('buildGroupRoomActionButton') || !appShell.includes('openForumSettingsDialog') || !appShell.includes('syncForumAutoTimer') || !appShell.includes('buildChatSummaryHistoryPage')) fail('缺少群组自动更新、逐群预设、可点击论坛频道、触屏/滚轮刷新或本地总结档案 UI 接线');
if (!forumService.includes('每次刷新都必须且只能生成') || !forumService.includes('顶部替换刷新') || !forumService.includes('底部追加刷新') || !forumService.includes('generateForumExistingPostsUpdate') || !forumService.includes('isKnownForumChannelTopic')) fail('缺少论坛全频道刷新、顶部替换/底部追加、既有帖子更新提示词或严格频道校验');
...
for (const [name, serviceMarker] of [
    ['generateGroupConversationUpdate', 'generateGroupChatUpdateService'],
    ['generateForumHomeRefresh', 'generateForumHomeRefreshService'],
    ['generateForumExistingPostsUpdate', 'generateForumExistingPostsUpdateService'],
    ['generateForumPostConversationUpdate', 'generateForumPostConversationUpdateService'],
    ['generateLocalGroupForumSummary', 'generateLocalConversationSummaryService'],
]) {
    const methodSource = extractBridgeMethod(actionBridge, name);
    if (!methodSource || !methodSource.includes('readLatestState') || !methodSource.includes(serviceMarker)) fail(`缺少 ${name} 的只读本地桥接`);
    if (/\b(?:applyControlledPatch|replaceMvuData|parseMessage|replaceVariables)\s*\(/.test(methodSource)) fail(`${name} 不得通过受控 Patch 或 MVU 写入`);
}
...
if (!appShell.includes('buildMessagesPage') || !appShell.includes('buildPrivateChatPage') || !appShell.includes('buildChatSummarySettingsHome') || !appShell.includes('buildChatSummaryHistoryPage') || !appShell.includes('buildPrivateChatSummaryPage') || !appShell.includes('showSummaryToast') || !appShell.includes('openPrivateChat') || !appShell.includes('messageSearchQuery') || !appShell.includes('buildPrivateChatConfirmation') || !appShell.includes('yl-private-chat-more-menu') || !uiModel.includes('private_chat:')) fail('缺少私聊搜索、总结设置/档案、删除确认、会话子页或消息路由接线');
...
if (!appShell.includes('settings_images') || !appShell.includes('resolveImage') || !appShell.includes('图片素材')) fail('缺少图片管理路由或首页/公开资料图片展示接线');
