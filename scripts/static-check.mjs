...
if (manifest.version !== '0.1.27') fail('manifest.version 必须与扩展版本 0.1.27 统一');
...
if (!appShell.includes("const UI_VERSION = '0.1.27'")) fail('关于软件 UI_VERSION 必须与扩展版本 0.1.27 统一');
...
if (!defaultPromptPresets.includes('builtin_recommendation_sfw') || !defaultPromptPresets.includes('builtin_private_chat_sfw') || !defaultPromptPresets.includes('builtin_chat_summary_sfw') || !defaultPromptPresets.includes('builtin_character_authoring_sfw')) fail('缺少本地可编辑的默认 SFW/NSFW 提示词预设');
...
if (!groupChatService.includes('generateGroupChatReply') || !groupChatService.includes('generateGroupChatUpdate') || !forumService.includes('generateForumPostDraft') || !forumService.includes('generateForumHomeRefresh') || !forumService.includes('generateForumPostConversationUpdate') || !groupLlmSafety.includes('buildPublicGroupLlmContext') || !groupLlmSafety.includes('isSafeGroupLlmOutput')) fail('缺少群聊/论坛更新服务或公开 LLM 安全投影');
if (!groupForumStore.includes('createGroupForumStore') || !groupForumStore.includes('GROUP_FORUM_STORAGE_KEY') || !groupForumStore.includes('FORUM_CHANNELS') || !groupForumStore.includes('isKnownForumChannelTopic') || !groupForumStore.includes('appendGroupModelUpdate') || !groupForumStore.includes('saveConversationSummary') || !groupForumStore.includes('NON_ADULT_PROFILE')) fail('缺少浏览器本地群组/论坛存储、频道或成年人校验边界');
...
console.log('✓ 阶段 49 群组/论坛本地持久化、可点击频道、触屏/滚轮下拉刷新、LLM 安全边界和只读桥接');
