...
if (manifest.version !== '0.1.28') fail('manifest.version 必须与扩展版本 0.1.28 统一');
...
if (!appShell.includes("const UI_VERSION = '0.1.28'")) fail('关于软件 UI_VERSION 必须与扩展版本 0.1.28 统一');
...
if (!groupForumStore.includes('createGroupForumStore') || !groupForumStore.includes('GROUP_FORUM_STORAGE_KEY') || !groupForumStore.includes('FORUM_CHANNELS') || !groupForumStore.includes('isKnownForumChannelTopic') || !groupForumStore.includes('appendGroupModelUpdate') || !groupForumStore.includes('clearGroupHistory') || !groupForumStore.includes('exitGroup') || !groupForumStore.includes('setForumAuto') || !groupForumStore.includes('updateExistingForumPosts') || !groupForumStore.includes('saveConversationSummary') || !groupForumStore.includes('NON_ADULT_PROFILE')) fail('缺少浏览器本地群组/论坛存储、频道或成年人校验边界');
...
if (!index.includes('createGroupForumStore') || !appShell.includes('bindForumPullToRefresh') || !appShell.includes('forumWheelPullState') || !appShell.includes('forumInteractionAbortController') || !appShell.includes('forumPostsForActiveChannel') || !appShell.includes("'data-forum-channel'") || !appShell.includes("'wheel'") || !appShell.includes('forumIsAtTop') || !appShell.includes('openGroupAutoDialog') || !appShell.includes('buildGroupRoomActionButton') || !appShell.includes('openForumSettingsDialog') || !appShell.includes('syncForumAutoTimer') || !appShell.includes('buildChatSummaryHistoryPage')) fail('缺少群组自动更新、可点击论坛频道、触屏/滚轮下拉刷新或本地总结档案 UI 接线');
if (!forumService.includes('每次刷新都必须且只能生成') || !forumService.includes('generateForumExistingPostsUpdate') || !forumService.includes('新的本地帖子只能由首页下拉刷新生成') || !forumService.includes('isKnownForumChannelTopic')) fail('缺少论坛全频道刷新、既有帖子自动更新提示词或严格频道校验');
...
console.log('✓ 阶段 50 群组独立自动更新、论坛既有帖子自动更新、可点击频道、下拉刷新、LLM 安全边界和只读桥接');
