...
const requiredFiles = [
    ...
    'src/chat/private-chat-response.js', 'src/chat/private-chat-service.js', 'src/chat/interaction-rhythm.js', 'src/chat/test/private-chat-response.test.mjs', 'src/chat/test/private-chat-service.test.mjs', 'src/chat/test/interaction-rhythm.test.mjs',
    ...
    'src/mvu/test/recommendation-refresh-patch.test.mjs', 'src/mvu/test/like-match-patch.test.mjs', 'src/mvu/test/private-chat-rhythm-patch.test.mjs', 'src/mvu/test/meetup-handoff.test.mjs', 'src/mvu/test/soul-preference-patch.test.mjs', 'src/mvu/test/player-public-profile-patch.test.mjs',
...
if (manifest.version !== '0.1.22') fail('manifest.version 必须与扩展版本 0.1.22 统一');
...
if (!appShell.includes("const UI_VERSION = '0.1.22'")) fail('关于软件 UI_VERSION 必须与扩展版本 0.1.22 统一');
...
if (!controlledPatch.includes('buildPrivateChatPatch') || !controlledPatch.includes('buildClearPrivateChatPatch') || !controlledPatch.includes('buildDeleteCharacterPatch') || !actionBridge.includes('runPrivateChat') || !actionBridge.includes('clearPrivateChat') || !actionBridge.includes('deleteCharacter')) fail('缺少私聊节奏/删除受控 Patch 或唯一 MVU 写入桥接');
if (!appShell.includes('buildMessagesPage') || !appShell.includes('buildPrivateChatPage') || !appShell.includes('openPrivateChat') || !appShell.includes('messageSearchQuery') || !appShell.includes('buildPrivateChatConfirmation') || !appShell.includes('yl-private-chat-more-menu') || !uiModel.includes('private_chat:')) fail('缺少私聊搜索、删除确认、会话子页或消息路由接线');
