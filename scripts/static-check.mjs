...
if (manifest.version !== '0.1.21') fail('manifest.version 必须与扩展版本 0.1.21 统一');
...
if (!appShell.includes("const UI_VERSION = '0.1.21'")) fail('关于软件 UI_VERSION 必须与扩展版本 0.1.21 统一');
...
if (!controlledPatch.includes('buildPrivateChatPatch') || !controlledPatch.includes('buildDeletePrivateChatPatch') || !actionBridge.includes('runPrivateChat') || !actionBridge.includes('deletePrivateChat')) fail('缺少私聊节奏/删除受控 Patch 或唯一 MVU 写入桥接');
if (!appShell.includes('buildMessagesPage') || !appShell.includes('buildPrivateChatPage') || !appShell.includes('openPrivateChat') || !appShell.includes('messageSearchQuery') || !appShell.includes('buildDeletePrivateChatConfirmation') || !uiModel.includes('private_chat:')) fail('缺少私聊搜索、删除确认、会话子页或消息路由接线');
