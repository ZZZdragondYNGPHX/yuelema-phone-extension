...
if (manifest.version !== '0.1.26') fail('manifest.version 必须与扩展版本 0.1.26 统一');
...
if (!appShell.includes("const UI_VERSION = '0.1.26'")) fail('关于软件 UI_VERSION 必须与扩展版本 0.1.26 统一');
...
if (!groupForumStore.includes('createGroupForumStore') || !groupForumStore.includes('GROUP_FORUM_STORAGE_KEY') || !groupForumStore.includes('appendGroupModelUpdate') || !groupForumStore.includes('saveConversationSummary') || !groupForumStore.includes('NON_ADULT_PROFILE')) fail('缺少浏览器本地群组/论坛存储、总结或成年人校验边界');
...
if (!index.includes('createGroupForumStore') || !appShell.includes('bindForumPullToRefresh') || !appShell.includes('forumWheelPullState') || !appShell.includes("'wheel'") || !appShell.includes('forumIsAtTop') || !appShell.includes('openGroupAutoDialog') || !appShell.includes('buildChatSummaryHistoryPage')) fail('缺少群组自动更新、触屏/滚轮论坛下拉刷新或本地总结档案 UI 接线');
...
console.log('✓ 阶段 48 群组/论坛本地持久化、触屏/滚轮下拉刷新、LLM 安全边界和只读桥接');
