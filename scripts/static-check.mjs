...
if (!index.includes('createGroupForumStore') || !appShell.includes('bindForumPullToRefresh') || !appShell.includes('forumWheelPullState') || !appShell.includes('forumInteractionAbortController') || !appShell.includes('forumPostsForActiveChannel') || !appShell.includes("'data-forum-channel'") || !appShell.includes("'wheel'") || !appShell.includes('forumIsAtTop') || !appShell.includes('forumIsAtBottom') || !appShell.includes('openGroupAutoDialog') || !appShell.includes('openGroupPresetDialog') || !appShell.includes('buildGroupRoomActionButton') || !appShell.includes('openForumSettingsDialog') || !appShell.includes('syncForumAutoTimer') || !appShell.includes('buildChatSummaryHistoryPage')) fail('缺少群组自动更新、逐群预设、可点击论坛频道、触屏/滚轮刷新或本地总结档案 UI 接线');
...
if (!settingsStore.includes('SETTINGS_SCHEMA_VERSION = 13') || !settingsStore.includes('getImageGenerationSettings') || !settingsStore.includes('getConversationImageGenerationSettings')) fail('缺少生图设置 schema 或逐会话自动生图隔离');
