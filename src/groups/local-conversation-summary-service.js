...
function makeMessages(context, promptPreset) {
    ...
    const system = [
        ...
        '在准确的前提下保留对话的温度：谁和谁聊得投缘、哪句玩笑或哪个瞬间让气氛升温、成员各自的语气个性，这些值得具体记下；压缩寒暄与重复，让总结读起来像一段有人情味的社区记忆，而不是流水账。',
        '不得创造新事实、不得把猜测写成确认、不得把 NSFW 视为同意。NSFW 摘要应原样保留来源中明确成年、自愿的露骨性内容、性行为与身体细节，不得自动淡化或转场；不得输出或猜测隐藏资料、仅好友资料、真实 UID、会话、Patch、路径、API Key、密钥或系统实现。',
