...
function repeatState({ status = '已完成' } = {}) {
    ...
    state.正文关系候选.npc_service_1 = createEmptyBodyRelationshipCandidate();
    state.关系叙事.npc_service_1 = createEmptyRelationshipNarrative();
...
test('service handoff atomically copies an adult local draft and opens one pending SFW order', () => {
    ...
    assert.deepEqual(built.value.patch.map(({ op, path }) => [op, path]), [
        ['add', '/角色池/npc_service_1'], ['add', '/正文记忆/npc_service_1'], ['add', '/关系叙事/npc_service_1'], ['add', '/正文关系候选/npc_service_1'], ['add', '/服务订单/service_1'],
        ['replace', '/系统/UID计数器/角色'], ['replace', '/系统/UID计数器/服务订单'],
    ]);
    assert.equal(built.value.patch[4].value.状态, '待确认');
    assert.equal(built.value.patch[4].value.服务分类, 'girl_shuren');
    assert.equal(built.value.patch[4].value.已确认边界, '');
    assert.deepEqual(built.value.patch[4].value.合法结束条件, { 已满足: false, 摘要: '', 记录时间: '' });
    assert.equal(validateControlledPatchAgainstState(state, built.value.patch).ok, true);
    const forged = structuredClone(built.value.patch); forged[4].value.状态 = '进行中';
    ...
    forgedCompletion[4].value.合法结束条件 = { 已满足: true, 摘要: '伪造完成', 记录时间: '现在' };
...
test('service handoff accepts every adult candidate the local generation gate accepts (no extra personal-name gate)', () => {
    ...
    for (const nickname of ['Momo酱', '小7', '咖啡师安然']) {
        ...
        assert.equal(built.value.patch[4].value.服务主题, `熟人商品：与${nickname}的文字协商`);
...
test('service lifecycle permits only exact start, cancel, complete, finalize, and rebook transitions', () => {
    ...
    pending.正文关系候选[handoff.value.npcUid] = handoff.value.patch[3].value;
    pending.关系叙事[handoff.value.npcUid] = handoff.value.patch[2].value;
    pending.服务订单[handoff.value.orderUid] = structuredClone(handoff.value.patch[4].value);
    ...
    cancelled.正文关系候选[cancelledHandoff.value.npcUid] = cancelledHandoff.value.patch[3].value;
    cancelled.关系叙事[cancelledHandoff.value.npcUid] = cancelledHandoff.value.patch[2].value;
    cancelled.服务订单[cancelledHandoff.value.orderUid] = structuredClone(cancelledHandoff.value.patch[4].value);
...
test('three-person handoff derives ordered service roles atomically and rejects a fourth or duplicated name', () => {
    ...
    assert.equal(built.value.orderUid, 'service_1');
    assert.deepEqual(built.value.patch.map(({ op, path }) => [op, path]), [
        ['add', '/角色池/npc_service_1'], ['add', '/正文记忆/npc_service_1'], ['add', '/关系叙事/npc_service_1'], ['add', '/正文关系候选/npc_service_1'],
        ['add', '/角色池/npc_service_2'], ['add', '/正文记忆/npc_service_2'], ['add', '/关系叙事/npc_service_2'], ['add', '/正文关系候选/npc_service_2'],
        ['add', '/角色池/npc_service_3'], ['add', '/正文记忆/npc_service_3'], ['add', '/关系叙事/npc_service_3'], ['add', '/正文关系候选/npc_service_3'],
        ['add', '/服务订单/service_1'], ['replace', '/系统/UID计数器/角色'], ['replace', '/系统/UID计数器/服务订单'],
    ]);
    assert.deepEqual(built.value.patch[12].value.角色UID列表, built.value.npcUids);
    assert.deepEqual(built.value.patch[12].value.合法结束条件, { 已满足: false, 摘要: '', 记录时间: '' });
    assert.equal(built.value.patch[13].value, 3);
    assert.equal(built.value.patch[14].value, 1);
    assert.equal(validateControlledPatchAgainstState(state, built.value.patch).ok, true);
...
test('multi-person terminal repeat and local rebook preserve the participant set but never stale contract fields', () => {
    ...
    state.正文关系候选.npc_service_1 = createEmptyBodyRelationshipCandidate();
    state.正文关系候选.npc_service_2 = createEmptyBodyRelationshipCandidate();
    state.关系叙事.npc_service_1 = createEmptyRelationshipNarrative();
    state.关系叙事.npc_service_2 = createEmptyRelationshipNarrative();
...
test('history deletion removes every isolated service role in one exact patch and refuses unsafe targets', () => {
    ...
    assert.deepEqual(deleted.value, [
        { op: 'remove', path: '/正文记忆/npc_service_1' }, { op: 'remove', path: '/正文关系候选/npc_service_1' }, { op: 'remove', path: '/关系叙事/npc_service_1' }, { op: 'remove', path: '/角色池/npc_service_1' },
        { op: 'remove', path: '/正文记忆/npc_service_2' }, { op: 'remove', path: '/正文关系候选/npc_service_2' }, { op: 'remove', path: '/关系叙事/npc_service_2' }, { op: 'remove', path: '/角色池/npc_service_2' },
    ]);
...
test('repair removes only malformed service orders, including over-three participant records', () => {
    ...
    state.正文记忆.npc_service_1 = '';
    state.正文关系候选.npc_service_1 = handoff.value.patch[3].value;
    state.关系叙事.npc_service_1 = handoff.value.patch[2].value;
    state.服务订单.service_1 = structuredClone(handoff.value.patch[4].value);
    state.系统.UID计数器.角色 = 1;
    state.系统.UID计数器.服务订单 = 1;
...
test('new controlled service patches accept every person category in either mode and reject legacy or unknown categories', () => {
    ...
    for (const mode of ['SFW', 'NSFW']) {
        for (const [categoryId, category] of Object.entries(personCategories[mode])) {
            ...
            assert.equal(built.value.patch[4].value.服务主题, `${category}：与林澈的文字协商`);
            assert.equal(validateControlledPatchAgainstState(state, built.value.patch).ok, true);
        }
    }
