...
function candidate() {
    return {
        ...
        与玩家关系: { 状态: '陌生', 全局账号表现: 60, NPC专属匹配度: 70, 好感: 0, 信任: 0, 戒备: 10, 面基意愿: 0 },
    };
}

function state() {
    return {
        系统: { UID计数器: { 角色: 12 } }, 软件: { 内容模式: 'SFW', 关于软件点击数: 0 }, 角色池: {}, 正文记忆: {}, 正文关系候选: {}, 关系叙事: {},
...
test('an existing custom candidate can be shown by refresh or promoted by a successful match through exact patches', () => {
    ...
    assert.deepEqual(matched.value.map((operation) => [operation.op, operation.path]), [
        ...
        ['add', '/关系叙事/npc_custom_13'],
        ['add', '/正文关系候选/npc_custom_13'],
