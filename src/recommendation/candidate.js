...
const LEGACY_RELATIONSHIP_KEYS = Object.freeze(['状态', '全局账号表现', 'NPC专属匹配度', '好感', '信任', '戒备', '面基意愿']);
const BOND_RELATIONSHIP_KEYS = Object.freeze(['友情值', '心动值', '欲望值']);
const COMPLETE_RELATIONSHIP_KEYS = Object.freeze([...LEGACY_RELATIONSHIP_KEYS, ...BOND_RELATIONSHIP_KEYS]);
...
function normalizeRelationship(value) {
    const hasAnyBondField = value !== null
        && typeof value === 'object'
        && BOND_RELATIONSHIP_KEYS.some(key => Object.hasOwn(value, key));
    const acceptedKeys = hasAnyBondField ? COMPLETE_RELATIONSHIP_KEYS : LEGACY_RELATIONSHIP_KEYS;
    assertRecord(value, acceptedKeys, '与玩家关系');
    ...
    for (const key of LEGACY_RELATIONSHIP_KEYS.slice(1)) {
        relationship[key] = normalizeInteger(ownData(value, key, '与玩家关系'), `与玩家关系.${key}`, 0, 100);
    }
    for (const key of BOND_RELATIONSHIP_KEYS) {
        const bondValue = hasAnyBondField
            ? normalizeInteger(ownData(value, key, '与玩家关系'), `与玩家关系.${key}`, 0, 100)
            : 0;
        if (bondValue !== 0) fail(`与玩家关系.${key}:not_zero_for_new_candidate`);
        relationship[key] = bondValue;
    }
    return relationship;
}
