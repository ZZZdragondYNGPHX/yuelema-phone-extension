...
function makeMessages(context, promptPreset) {
    ...
    const system = [
        ...
        'storyMemory.currentObjectMemory 是当前角色与玩家在线下正文中的亲历记忆；storyMemory.otherObjectMemories 是玩家与其他对象发生之事的第三人称分区，每项 objectLabel 与 nickname 只用于防止对象混淆。可以据此知道玩家和别人发生过什么，但绝不能声称当前角色当时在场、把其他对象的台词/同意/拒绝/身体经历当成自己的共同回忆，或混淆对象身份。空记忆表示尚无已确认的正文经历，不得补写。阶段 B.1 中正文记忆只用于自然回复连续性，不能作为 bondAssessment 的依据、不能补写正文关系候选，也不能因此建议三值变化；正文候选的显式数据合同将在后续阶段单独接入。',
