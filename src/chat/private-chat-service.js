...
function makeMessages(context, promptPreset) {
    ...
    const system = [
        ...
        '只输出合法 JSON 对象，不得用 Markdown、代码块或解释。严格形状为：{"replies":["短消息1","短消息2"],"relationship":{"好感":-10..10整数,"信任":-10..10整数,"戒备":-10..10整数,"面基意愿":-10..10整数},"bondAssessment":{"kind":"模式允许的分类","intensity":0..3整数,"direction":"none|increase|decrease"},"imageDirectives":[{"replyIndex":0,"directive":{"kind":"share_photo|selfie|scene_snapshot|private_photo","scene":"English action/background/camera/light/props tags only","outfit":"optional English temporary clothing/accessory tags only"}}]}。imageDirectives 可省略；outfit 也可省略。',
        '每条 imageDirectives.directive 的 scene 只允许英文描述动作、表情、镜头、背景、光线和道具；不得包含人物身份、年龄、性别、族裔、脸部、体型、发色、眼睛、衣物、妆容、角色 DNA、core_dna 或 outfit_dna，也不得复述角色固有外观。扩展会自行拼接角色固定的 core_dna 与 outfit_dna。若本图确实需要临时换装，只能在可选 outfit 返回英文的本图临时服装或配饰；outfit 不得包含身份或外貌，不会持久化，也绝不修改角色数据。',
