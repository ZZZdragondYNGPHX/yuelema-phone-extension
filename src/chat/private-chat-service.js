...
function normalizeVisibleMessages(value) {
    ...
    const messages = [];
    for (const raw of value.slice(-MAX_CONTEXT_MESSAGES)) {
        if (!ownRecord(raw) || !['玩家', '角色', '系统'].includes(raw.发送者)) continue;
        const content = cleanText(raw.内容, 600);
        if (!content) continue;
        messages.push(Object.freeze({ sender: raw.发送者, content }));
    }
...
export function buildPrivateChatContext({ state, sessionUid, npcUid, playerMessage } = {}) {
    ...
    return {
        ...
        context: Object.freeze({
            ...
            recentMessages: normalizeVisibleMessages(session.最近消息),
...
export async function generatePrivateChatReply({ state, sessionUid, npcUid, playerMessage, settingsStore, llmClient, signal } = {}) {
    const builtContext = buildPrivateChatContext({ state, sessionUid, npcUid, playerMessage });
