...
test('本地 PNG/JPEG/WebP 通过注入压缩器导入，且回调结果只保存嵌入来源', async () => {
    ...
    try {
        ...
        const records = await harness.store.list();
        assert.deepEqual(records[0].source, { kind: 'embedded', dataUrl: 'data:image/webp;base64,QUJDRA==' });
        assert.equal(harness.element.querySelector('.yl-image-preview').getAttribute('src'), 'data:image/webp;base64,QUJDRA==');
    } finally {
        harness.dispose();
    }
});
