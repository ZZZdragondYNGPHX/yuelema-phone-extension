...
`settings-store.js` 管理版本化数据（当前 `schema: "yuelema.settings"`、`schemaVersion: 2`）：

- 非机密连接预设：`id`、`name`、`url`、`model`、`temperature`、`maxTokens`、`timeoutMs`、`transportMode`；
- Worldbook 风格提示词预设及其条目数据；
- 默认连接/提示词预设 ID；
- `chat`、`character_authoring`、`soul_match`、`text_match`、`recommendation_refresh`、`group_chat`、`forum` 的功能绑定；
- 个性化内容推荐开关与关键词权重。

`transportMode` 仅允许 `json`、`stream`、`pseudo_stream`。旧 schema v1 和旧 v2 连接预设缺失该字段时都会安全归一化为 `json`，导出后固定带当前字段；这不会放宽保存预设时 model 必须非空的校验。
...
API Key 仍只能由 `../llm/session-key-store.js` 放入 ES 模块闭包的本次会话内存中。本目录没有解锁、读取、序列化或导出密钥的接口。
...
- 第一个添加的同类预设成为默认预设；功能绑定未指定的部分回退到默认值。
...
- `importJson(json)` 支持 schema v1 到 v2 的既有迁移，以及旧 v2 连接预设补全 `transportMode: "json"`。
...
1. 实际 localStorage 命名空间、配额和 schema v2 旧数据迁移。
2. 设置 UI 对三个传输模式的编辑、保存、重载与导入导出。
3. 模型列表拉取和功能绑定是否始终不把密钥带入 DOM、日志或导出文件。
