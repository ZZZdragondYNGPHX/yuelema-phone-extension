...
const BUILTIN_PROMPT_PRESETS = Object.freeze([
    ...
    Object.freeze({
        ...
        name: '内置·描述匹配·SFW',
        contentMode: 'SFW',
        content: '只从用户本次主动输入的匹配描述中提炼适合描述匹配的关键词与权重，不复述原文，不猜测隐私。描述匹配是纯关键词驱动：只使用本次提炼的临时关键词权重与本地保存的关键词权重进行匹配，不读取、不考虑、不假设玩家的性别、性取向或其他个人资料。关键词应具体、友善且适合成年人线上交友。本模式保持日常社交尺度，不输出现实线下行为内容。不得输出隐藏资料、仅好友资料、关系数值、会话、UID、Patch、密钥或系统信息。',
        ...PRESET_LAYOUT,
    }),
    Object.freeze({
