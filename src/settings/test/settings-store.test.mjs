...
test('默认内存存储与预设 CRUD、默认策略、功能回退', () => {
    ...
    assert.equal(initial.promptPresets.length, 16);
    assert.ok(initial.promptPresets.every((preset) => preset.id.startsWith('builtin_')));
    assert.equal(initial.promptPresets.filter((preset) => preset.contentMode === 'SFW').length, 8);
    assert.equal(initial.promptPresets.filter((preset) => preset.contentMode === 'NSFW').length, 8);
...
test('schema v1 设置可安全迁移到 v6 个性化字段、默认提示词、模式绑定与总结设置，导出固定为当前版本', () => {
    const legacy = {
        schema: 'yuelema.settings',
        schemaVersion: 1,
    ...
    assert.equal(migrated.promptPresets.filter((preset) => preset.id.startsWith('builtin_')).length, 16);
    ...
    assert.equal(JSON.parse(store.exportJson()).schemaVersion, 6);
});

test('schema v3 迁移补齐提示词模式、历史跨模式绑定并升级到 v6', () => {
...
test('清理会重建可编辑默认预设且不触及会话密钥模块', () => {
    ...
    assert.equal(cleared.promptPresets.length, 16);
});
