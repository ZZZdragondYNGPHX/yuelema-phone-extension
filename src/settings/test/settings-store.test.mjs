...
test('旧 settings schema v1-v10 统一拒绝且不覆盖当前 v19 设置', () => {
...
test('v11-v14 设置在加载与导入时迁移到 v19：内置提示词刷新为新文案，自定义预设保留', () => {
    for (const legacyVersion of [11, 12, 13, 14]) {
        ...
        assert.equal(JSON.parse(seeded.getItem(SETTINGS_STORAGE_KEY)).schemaVersion, 19, '迁移结果必须落盘为 v19');
...
test('v15→v19 迁移补入独立 ComfyUI 配置、刷新服务内置提示词且不改写自定义提示词', () => {
    const seedStore = createSettingsStore({ storage: createMemoryStorage() });
    seedStore.load();
...
test('v16→v19 只刷新仍存在的服务内置提示词，不复活删除项或改写自定义预设', () => {
    const seedStore = createSettingsStore({ storage: createMemoryStorage() });
    seedStore.load();
    ...
    assert.equal(JSON.parse(seeded.getItem(SETTINGS_STORAGE_KEY)).schemaVersion, 19);
});

test('v17→v19 复制既有通用提示词到 OpenAI 专属预设并保留 NAI 值', () => {
    const seedStore = createSettingsStore({ storage: createMemoryStorage() });
...
test('v18→v19 保留 OpenAI 专属配置并从旧共用尺寸初始化独立尺寸', () => {
    const seedStore = createSettingsStore({ storage: createMemoryStorage() });
    const legacy = seedStore.load();
