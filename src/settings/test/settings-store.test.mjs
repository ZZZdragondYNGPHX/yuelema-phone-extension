...
test('默认内存存储与预设 CRUD、默认策略、功能回退', () => {
    ...
    assert.equal(initial.promptPresets.length, 8);
...
test('个性化内容推荐设置本地持久化，关键词权重严格校验且不接入功能绑定', () => {
    const storage = createMemoryStorage();
    const store = createSettingsStore({ storage });
...
test('schema v1 设置可安全迁移到 v3 个性化字段、默认提示词与模式绑定，导出固定为当前版本', () => {
    const legacy = {
        schema: 'yuelema.settings',
        schemaVersion: 1,
    ...
    assert.equal(migrated.promptPresets.filter((preset) => preset.id.startsWith('builtin_')).length, 8);
    assert.equal(store.resolveFunction('chat', { contentMode: 'SFW' }).promptPreset.id, 'base', '已有默认提示词应优先保留');
    assert.equal(JSON.parse(store.exportJson()).schemaVersion, 3);
});
...
test('清理会重建可编辑默认预设且不触及会话密钥模块', () => {
    ...
    assert.equal(cleared.promptPresets.length, 8);
});
