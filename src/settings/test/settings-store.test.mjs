...
test('默认内存存储与预设 CRUD、默认策略、功能回退', () => {
    ...
    assert.equal(initial.schemaVersion, 18);
...
test('旧 settings schema v1-v10 统一拒绝且不覆盖当前 v18 设置', () => {
...
test('v11-v14 设置在加载与导入时迁移到 v18：内置提示词刷新为新文案，自定义预设保留', () => {
    for (const legacyVersion of [11, 12, 13, 14]) {
        ...
        assert.equal(JSON.parse(seeded.getItem(SETTINGS_STORAGE_KEY)).schemaVersion, 18, '迁移结果必须落盘为 v18');
...
test('v15→v18 迁移补入独立 ComfyUI 配置、刷新服务内置提示词且不改写自定义提示词', () => {
    const seedStore = createSettingsStore({ storage: createMemoryStorage() });
    seedStore.load();
...
test('v16→v18 只刷新仍存在的服务内置提示词，不复活删除项或改写自定义预设', () => {
    const seedStore = createSettingsStore({ storage: createMemoryStorage() });
    seedStore.load();
    ...
    assert.equal(JSON.parse(seeded.getItem(SETTINGS_STORAGE_KEY)).schemaVersion, 18);
});

test('v17→v18 复制既有通用提示词到 OpenAI 专属预设并保留 NAI 值', () => {
    const seedStore = createSettingsStore({ storage: createMemoryStorage() });
