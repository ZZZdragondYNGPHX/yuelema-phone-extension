...
test('schema v1 设置可安全迁移到 v7 个性化字段、默认提示词、模式绑定与总结设置，导出固定为当前版本', () => {
    const legacy = {
        schema: 'yuelema.settings',
        schemaVersion: 1,
    ...
    assert.equal(JSON.parse(store.exportJson()).schemaVersion, 7);
});

test('schema v3 迁移补齐提示词模式、历史跨模式绑定并升级到 v7', () => {
