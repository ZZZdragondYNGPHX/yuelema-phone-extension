...
import { clearSessionKeys, hasSessionKey } from '../../llm/session-key-store.js';
...
test('填写 API Key 后保存连接预设会立即解锁本次会话，但不会写入本地设置', async () => {
    clearSessionKeys();
    try {
        ...
        byAria(panel, 'API Key，仅本次会话').value = 'session-only-secret';
        await click(button(panel, '保存连接预设'));

        const saved = store.snapshot().connectionPresets[0];
        assert.ok(saved);
        assert.equal(hasSessionKey(saved.id), true);
        assert.equal(byAria(panel, 'API Key，仅本次会话').value, '');
        assert.equal(store.exportJson().includes('session-only-secret'), false);
        assert.ok(feedback.some((message) => message.includes('已解锁本次会话')));
    } finally {
        clearSessionKeys();
    }
});
...
test('Model 为空时也能解锁并从 /models 拉取模型列表', async () => {
    ...
    byAria(panel, 'API Key，仅本次会话').value = 'session-secret';
    try {
...
test('提示词详情不混入连接设置，文案去掉风格措辞且可安全导入导出', async () => {
    ...
    assert.equal(importedStore.snapshot().promptPresets.length, 13);
