# 约了吗小手机 v0.1.8（阶段 30 模式化预设与本地默认提示词，静态 / Node 验证）
...
> 连接预设、提示词预设和功能绑定可保存在本浏览器；首次使用会写入八份可编辑的默认提示词预设，重开连接页会自动载入默认连接预设。API Key 只在当前扩展会话解锁，不保存、导出、写入 MVU 或回显。真实认证、CORS、模型接口与扩展生命周期仍待目标酒馆版本验证。
...
- OpenAI-compatible 非机密连接预设、提示词预设（`depth / order / position / enabled / content`）、模型拉取及按功能绑定。首次加载会把推荐人物、私聊、AI 补全、AI 完整创作各自的 SFW / NSFW 默认预设写入本地提示词预设列表，之后可直接查看、编辑、删除或导出；连接预设与提示词预设字段相互隔离；API Key 仅模块内存解锁。
- 首页、匹配、消息、群组、AI 补全与完整创作均可通过右上角“设置”分别绑定连接／提示词预设。绑定按 SFW / NSFW 分开保存；切换内容模式后，已打开的设置弹窗和所有入口都会显示当前模式对应的一套绑定。旧角色创作绑定会兼容迁移到两个新入口。
...
- 连接预设支持 `json` 普通响应、`stream` SSE 真流式和 `pseudo_stream` 假流式。拉取模型列表使用独立的宽松探测校验，Model 可留空；正式保存和对话请求仍要求已选模型。SSE 会处理跨 chunk 分行、`[DONE]`、文本数组、超时、取消和 2 MiB 响应上限；假流式只改变完整响应后的展示元数据，不能绕过服务端输出上限。首页推荐刷新会至少申请 2048 Token，并兼容单层 JSON 代码围栏，避免完整候选对象被低额度或格式包装误判。
...
4. 在角色管理页面导入 `D:\Dev\AI制卡\约了吗\角色卡源\dist\约了吗_MVU_v0.1.8.json`，并为这张卡**新开聊天**；不要用旧聊天的变量状态代替初始化验收。
...
实际结果（阶段 30）：`npm run check`、全量 `node --test`、生产 ESM import 图与 `git diff --check` 均通过；全量 Node 回归 **216/216** 通过。新增回归覆盖默认提示词首次持久化、SFW / NSFW 独立绑定、角色两种 AI 独立绑定、以及已打开功能设置在模式切换后刷新到另一套预设。推荐刷新仍覆盖最小 2048 Token 请求、完整 JSON 验证、单层 JSON 代码围栏兼容、成年人 / 公开资料边界与失败不写入。
...
7. 本次只推送 GitHub 仓库，不同步本地安装副本；请在通过 Git URL 安装/更新后，再确认“关于软件”显示 0.1.8，并测试默认提示词是否出现在提示词预设列表、SFW / NSFW 切换后设置入口是否切换绑定、首页刷新、完整候选显示、失败不写入、连接预设重开后仍保留，以及消息列表、返回、窄屏输入、发送、回复中状态和刷新后的聊天记录。
