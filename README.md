# 约了吗小手机 v0.1.14（阶段 36 开放关键词学习与基础匹配，静态 / Node 验证）
...
4. 在角色管理页面导入 `D:\Dev\AI制卡\约了吗\角色卡源\dist\约了吗_MVU_v0.1.14.json`，并为这张卡**新开聊天**；不要用旧聊天的变量状态代替初始化验收。
...
实际结果（阶段 36）：`npm run check`、全量 `node --test`、生产 ESM import 图与 `git diff --check` 均通过；新增回归覆盖开放关键词库完整传入、固定主题移除、标准性别 / 性取向双向兼容校验、候选成功写回后未知公开标签以 `0` 收录，以及失败时不污染本地学习库。仍不绕过 `parseMessage → replaceMvuData → VARIABLE_UPDATE_ENDED`；推荐刷新继续覆盖最小 2048 Token 请求、完整 JSON 验证、单层 JSON 代码围栏兼容、成年人 / 公开资料边界与失败不写入。
...
7. 本次只推送 GitHub 仓库，不同步本地安装副本；请在通过 Git URL 安装/更新后，再确认“关于软件”显示 0.1.14。对收藏资料分别点击喜欢和不喜欢，确认均能完成受控写入且“个性化内容偏好”中的公开标签权重随之变化；连续刷新时检查新鲜标签会自动以 `0` 出现在词库中，已有正 / 负权重会软性影响候选但不限制 AI 的开放创作，并分别在 SFW / NSFW 检查性别 / 性取向基础匹配和公开标签规则。再测试灵魂匹配、语音匹配、私聊和角色 AI；确认同一连接仍可请求、删除当前 Key 后会要求重新填写、失败不写入，且最终 DOM / 网络请求 / 错误面板不含 Key，以及消息列表、返回、窄屏输入、发送、回复中状态和刷新后的聊天记录。
