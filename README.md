# 约了吗小手机 v0.1.18（阶段 40 匹配合同兼容与私聊纸飞机工具栏）
...
- “消息”一级页是独立会话列表；点入后进入深色霓虹风格的私聊详情，含联系人卡、聊天气泡、会话内回复中状态与加长的对话区域。输入栏右侧改为 Telegram 风格纸飞机：左键发送，右键打开工具栏；工具栏当前只有“约定面基”，点击后才展开原面基表单。点击私聊对象头像会打开其公开资料。发送与面基仍分别复用既有受控桥，面基只填入正文输入框、不自动发送。
...
4. 在角色管理页面导入 `D:\Dev\AI制卡\约了吗\角色卡源\dist\约了吗_MVU_v0.1.18.json`，并为这张卡**新开聊天**；不要用旧聊天的变量状态代替初始化验收。
...
实际结果（阶段 40）：`npm run check`、全量 `node --test`、生产 ESM import 图与 `git diff --check` 均通过。灵魂/语音候选第二阶段会隔离旧内置关键词/筛选提示词，避免与公开候选合同冲突；同时只兼容纯 JSON、完整单层 JSON 围栏或前后短说明中的唯一平衡根对象，多个并列对象仍拒绝。严格成年人、闭合字段、隐私与敏感键校验没有放宽，失败仍不进入 MVU 写入边界。私聊 DOM 回归覆盖纸飞机左键发送、右键工具栏、面基表单按需展开及页面/会话切换清理。
...
7. 本次只推送 GitHub 仓库，不手工同步本地安装副本；请通过 Git URL 更新后确认“关于软件”显示 0.1.18。重点复验灵魂匹配与语音匹配：旧内置提示词不应再导致公开资料安全格式失败，纯 JSON、单层 JSON 围栏及唯一对象前后短说明都应生成候选；多个对象、数组根或不安全字段仍必须拒绝且状态不变。私聊需确认对话区更长、底部不再常驻面基卡、纸飞机左键发送、右键只显示“约定面基”，点击后沿用原表单且只填入正文草稿、不自动发送。移动端长按与工具栏定位仍需真机观察；最终 DOM / 网络请求 / 错误面板不得出现 API Key、隐藏层、仅好友层、关系分、阈值、UID、Patch、原始响应或 `stat_data`。
