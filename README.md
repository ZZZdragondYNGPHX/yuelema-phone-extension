...
- 角色创建入口已迁移到“我的”。“我的 → 设置 → 图片管理”提供浏览器本地图片库：可导入本地 PNG/JPEG/WebP（压缩后保存）或 HTTP/HTTPS 图片链接，显示预览与加载失败状态；每张图可维护 `-5…5` 整数关键词权重，通过桌面右键或移动端长按打开“编辑匹配关键词”，并可在编辑窗口删除图片。`image_match` 使用独立连接/提示词绑定，只读取候选人的公开资料与图片库已有关键词；LLM 失败时回退本地关键词裁决。首页候选卡背景、首页头像和候选公开资料头像使用最高匹配图片，无结果时保留原渐变与姓名首字头像。
