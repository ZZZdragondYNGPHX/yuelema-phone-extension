# 约了吗小手机 v0.1.6（阶段 28 私聊界面静态 / Node 验证）
...
4. 在角色管理页面导入 `D:\Dev\AI制卡\约了吗\角色卡源\dist\约了吗_MVU_v0.1.6.json`，并为这张卡**新开聊天**；不要用旧聊天的变量状态代替初始化验收。
...
实际结果（阶段 28）：`npm run check`、全量 `node --test`、生产 ESM import 图与 `git diff --check` 均通过；全量 Node 回归 **210/210** 通过。私聊 UI 回归覆盖消息列表与详情页分离、公开投影/会话可见消息边界、受控私聊桥调用、回复中状态、成功清稿、失败保稿、离开会话后的迟到失败静默，以及 Enter / Shift+Enter 行为。
...
7. 本次只推送 GitHub 仓库，不同步本地安装副本；请在通过 Git URL 安装/更新后，再确认“关于软件”显示 0.1.6，并测试消息列表、返回、窄屏输入、发送、回复中状态和刷新后的聊天记录。
