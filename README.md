# 约了吗小手机 v0.1.7（阶段 29 推荐刷新完整 JSON 修复，静态 / Node 验证）
...
> 连接预设、提示词预设和功能绑定可保存在本浏览器；重开连接页会自动载入默认连接预设。API Key 只在当前扩展会话解锁，不保存、导出、写入 MVU 或回显。真实认证、CORS、模型接口与扩展生命周期仍待目标酒馆版本验证。
...
4. 在角色管理页面导入 `D:\Dev\AI制卡\约了吗\角色卡源\dist\约了吗_MVU_v0.1.7.json`，并为这张卡**新开聊天**；不要用旧聊天的变量状态代替初始化验收。
...
实际结果（阶段 29）：`npm run check`、全量 `node --test`、生产 ESM import 图与 `git diff --check` 均通过；全量 Node 回归 **212/212** 通过。推荐刷新回归覆盖最小 2048 Token 请求、完整 JSON 验证、单层 JSON 代码围栏兼容、成年人 / 公开资料边界与失败不写入。
...
7. 本次只推送 GitHub 仓库，不同步本地安装副本；请在通过 Git URL 安装/更新后，再确认“关于软件”显示 0.1.7，并测试首页刷新、完整候选显示、失败不写入、连接预设重开后仍保留，以及消息列表、返回、窄屏输入、发送、回复中状态和刷新后的聊天记录。
