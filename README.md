# 约了吗小手机 v0.1.16（阶段 38 推荐反馈、独立匹配会话与本地头像，静态 / Node 验证）
...
- 首页只保留毛玻璃候选角色卡：公开资料、关键词与喜欢 / 不喜欢 / 收藏 / 刷新四个操作；快速模型候选的昵称必须是自然人的个人姓名，不能是玩家、AI、系统、职业或概念标签。首页的喜欢/不喜欢只学习公开标签偏好，不会建立匹配或会话；收藏把对象留入收藏夹。空池卡片的刷新调用 `recommendation_refresh` 绑定的快速模型。
...
4. 在角色管理页面导入 `D:\Dev\AI制卡\约了吗\角色卡源\dist\约了吗_MVU_v0.1.16.json`，并为这张卡**新开聊天**；不要用旧聊天的变量状态代替初始化验收。
...
实际结果（阶段 38）：`npm run check`、全量 `node --test`、生产 ESM import 图与 `git diff --check` 均通过；新增回归覆盖首页反馈不建会话、收藏私聊的接受/婉拒阈值分支、灵魂/语音独立新角色与会话、语音临时权重优先级、私聊头像公开资料跳转，以及独立本地头像的导入/链接/移除。仍不绕过 `parseMessage → replaceMvuData → VARIABLE_UPDATE_ENDED`；推荐刷新继续覆盖最小 2048 Token 请求、完整 JSON 验证、单层 JSON 代码围栏兼容、成年人 / 公开资料边界与失败不写入。
...
7. 本次只推送 GitHub 仓库，不同步本地安装副本；请在通过 Git URL 安装/更新后，再确认“关于软件”显示 0.1.16。首页连续刷新应只显示自然个人姓名，喜欢/不喜欢只调整标签权重；收藏后确认只出现“取消收藏/发起私聊”，并分别验证低于角色拒绝阈值不会进入消息、高于阈值才建立私聊。再分别在 SFW / NSFW 验证灵魂匹配、语音匹配各自产生独立的互相喜欢与消息对象、私聊头像跳公开资料、我的头像只保留本浏览器，以及角色 AI；同一连接仍可请求、删除当前 Key 后会要求重新填写、失败不写入，且最终 DOM / 网络请求 / 错误面板不含 Key，以及消息列表、返回、窄屏输入、发送、回复中状态和刷新后的聊天记录。
