# 约了吗小手机 v0.1.13（阶段 35 推荐偏好反馈与结构化输出，静态 / Node 验证）
...
- 个性化内容推荐设置位于“设置 → 隐私权限设置 → 个性化内容推荐管理 → 个性化内容偏好”；关键词权重保存在当前设备。喜欢 / 收藏 / 不喜欢在受控 MVU 写入成功后会按候选的公开标签同步本地权重（分别为 +3 / +1 / -3，范围 -5…5）；关闭个性化后不会继续累积。本地权重会实际参与首页推荐：探索主题优先轮换，每第三轮才软性回访高权重标签，同时避开低权重与近期重复组合。灵魂匹配和语音匹配仅以该权重生成内存中的公开候选档案，不写入 MVU；语音匹配产生的同名临时权重优先于本地权重，原始输入不会持久化或显示。
...
4. 在角色管理页面导入 `D:\Dev\AI制卡\约了吗\角色卡源\dist\约了吗_MVU_v0.1.13.json`，并为这张卡**新开聊天**；不要用旧聊天的变量状态代替初始化验收。
...
实际结果（阶段 35）：`npm run check`、全量 `node --test`、生产 ESM import 图与 `git diff --check` 均通过；新增回归覆盖收藏卡的喜欢 / 不喜欢不再因已离开当前队列被拒绝、成功提交后公开标签同步到设备偏好、探索优先与每第三轮偏好回访，以及推荐 / 匹配 / 角色 AI 的系统级结构合同。仍不绕过 `parseMessage → replaceMvuData → VARIABLE_UPDATE_ENDED`；推荐刷新继续覆盖最小 2048 Token 请求、完整 JSON 验证、单层 JSON 代码围栏兼容、成年人 / 公开资料边界与失败不写入。
...
7. 本次只推送 GitHub 仓库，不同步本地安装副本；请在通过 Git URL 安装/更新后，再确认“关于软件”显示 0.1.13。对收藏资料分别点击喜欢和不喜欢，确认均能完成受控写入且“个性化内容偏好”中的公开标签权重随之变化；在 SFW / NSFW 各刷新多次，确认探索主题先轮换、偏好标签只间隔性出现、两种模式公开标签规则仍各自生效。再测试灵魂匹配、语音匹配、私聊和角色 AI；确认同一连接仍可请求、删除当前 Key 后会要求重新填写、失败不写入，且最终 DOM / 网络请求 / 错误面板不含 Key，以及消息列表、返回、窄屏输入、发送、回复中状态和刷新后的聊天记录。
