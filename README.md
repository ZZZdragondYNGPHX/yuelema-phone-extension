# 约了吗小手机 v0.1.9（阶段 31 会话解锁修复与模式提示词筛选，静态 / Node 验证）
...
4. 在角色管理页面导入 `D:\Dev\AI制卡\约了吗\角色卡源\dist\约了吗_MVU_v0.1.9.json`，并为这张卡**新开聊天**；不要用旧聊天的变量状态代替初始化验收。
5. 首次使用时在“我的 → 设置”配置 OpenAI-compatible 连接预设、提示词预设和功能绑定；输入 API Key 后点击“保存连接预设”会立即解锁该连接的**本次会话**，不必再额外点模型拉取按钮。Key 不会写入浏览器存储；禁用或删除扩展时，`disable / delete` hook 会立即清除内存中的已解锁 Key，非机密预设与模板仍按本地浏览器存储策略保留。
...
实际结果（阶段 31）：`npm run check`、全量 `node --test`、生产 ESM import 图与 `git diff --check` 均通过；全量 Node 回归 **220/220** 通过。新增回归覆盖“保存连接预设”即时会话解锁且 Key 不落盘、v3 设置迁移、灵魂/语音匹配默认提示词、NSFW 标记与跨模式绑定拒绝、以及已打开功能设置在模式切换后刷新到另一套预设。推荐刷新仍覆盖最小 2048 Token 请求、完整 JSON 验证、单层 JSON 代码围栏兼容、成年人 / 公开资料边界与失败不写入。
...
7. 本次只推送 GitHub 仓库，不同步本地安装副本；请在通过 Git URL 安装/更新后，再确认“关于软件”显示 0.1.9，检查 12 份默认提示词、每个提示词的 NSFW 标记与 SFW / NSFW 筛选。输入 API Key 并点击“保存连接预设”后，直接测试首页刷新、灵魂匹配、语音匹配、私聊和角色 AI；确认成功请求可用、失败不写入、最终 DOM / 网络请求 / 错误面板不含 Key，以及消息列表、返回、窄屏输入、发送、回复中状态和刷新后的聊天记录。
