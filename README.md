# 约了吗小手机 v0.1.17（阶段 39 推荐响应 JSON 前后缀容错，静态 / Node / 浏览器验证）
...
4. 在角色管理页面导入 `D:\Dev\AI制卡\约了吗\角色卡源\dist\约了吗_MVU_v0.1.17.json`，并为这张卡**新开聊天**；不要用旧聊天的变量状态代替初始化验收。
...
实际结果（阶段 39）：`npm run check`、全量 `node --test`、生产 ESM import 图与 `git diff --check` 均通过；推荐刷新新增回归覆盖兼容模型在唯一完整 JSON 对象前后附带简短说明、字符串内花括号和转义引号，以及截断 JSON、数组根、多个根对象和 `candidate` 包装对象的拒绝。恢复出的对象仍必须通过同一套成年人、闭合 Schema、隐私、姓名和双向基础匹配校验；失败仍不进入 MVU 写入边界。
...
7. 本次只推送 GitHub 仓库，不手工同步本地安装副本；请通过 Git URL 更新后确认“关于软件”显示 0.1.17。重点重复首页刷新：纯 JSON、单层 JSON 围栏，以及模型偶发在唯一完整对象前后附加简短说明时都应继续生成；截断、多个对象、数组根或包装对象仍必须报安全错误并保持原推荐不变。随后继续按既有清单验证推荐反馈、收藏私聊、灵魂/语音匹配、本地头像和私聊流程，且最终 DOM / 网络请求 / 错误面板不得出现 API Key、隐藏层、仅好友层、关系分、阈值、UID、Patch、原始响应或 `stat_data`。
